...
見た目は iPadOS の標準アプリと区別がつかないところを狙う。独自の意匠は持たない。
...
システムカラーとセマンティックカラーのみを使う。独自の色は定義しない。
ダークモードでは各色の iOS ダーク版へ自動的に切り替わる。

- **Primary (#007AFF / ダーク #0A84FF):** 操作できるものすべて。ボタン、リンク、選択状態、アイコン。
...
- **Grouped Background (#F2F2F7) と Cell (#FFFFFF):** 地とカードの2層構造。
  枠線ではなく、この明度差と 0.5px の区切り線だけで領域を分ける。
...
- **インセットグループドリスト:** 一覧、設定、ページ内の属性行。値は右端、押せる行には山形記号を付ける。

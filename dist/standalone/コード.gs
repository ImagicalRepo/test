...
var DEFAULT_SETTINGS = [
  ...
  ['サンプルを入れる', 'ON', '［初期セットアップ］でサンプルの業務を入れるか。配る用のシートでは OFF にする（メニューの［配布用に整える］が自動で OFF にします）'],
 ...
 * 基準日ルールの解釈と展開（純粋関数）
 *
 * 「毎月第2水曜日」のようなルールから、実際の基準日を自動生成する。
 *
 * 対応する書き方（全角/半角・空白は無視）:
...
function seedSamples_() {
  // どの部署でも読める言い方にしてある。実際の業務名・工程名に置き換えて使う。
  // 3業務で「毎月・第N曜日」「毎年・同じ日」「別サイクル」の3通りを見せている。
  appendRows_(SHEET.WORK, [
    {
      '業務ID': 'W1', '業務名': '月次の審査・通知発送', '有効': 'ON',
      '基準日名称': '会議', '基準日ルール': '毎月第2水', '基準日休日補正': '前営業日',
      '色': '青', '備考': '会議の日を基準に、前後の工程を自動算出'
    },
    {
      '業務ID': 'W2', '業務名': '年次の一斉更新', '有効': 'ON',
      '基準日名称': '受付開始', '基準日ルール': '毎年9月1日', '基準日休日補正': '翌営業日',
      '色': '緑', '備考': '年に1回。受付開始日を基準に前後を算出'
    },
    {
      '業務ID': 'W3', '業務名': '別サイクルの月次処理', '有効': 'ON',
      '基準日名称': '会議', '基準日ルール': '毎月第4火', '基準日休日補正': '前営業日',
      '色': '橙', '備考': '上とは別の周期で回る会議'
    }
  ]);

  var monthly = [
    [10, '受付分の締切（当月審査分）', '', '前', 20, '営業日', 3, '窓口・郵送分をここで締める'],
    [20, '形式審査・不備照会の完了', '', '前', 15, '営業日', 3, '不備は返戻または追加提出を依頼'],
    [30, '会議資料の作成・システム入力', '', '前', 10, '営業日', 3, '入力はここまでに終わらせる'],
    [40, '資料の最終確認（係内）', '', '前', 8, '営業日', 2, ''],
    [50, '委員へ資料送付', '', '前', 7, '営業日', 2, '発送日'],
    [60, '委員からの意見返送期限', '', '前', 3, '営業日', 2, '未着はここで督促'],
    [70, '会議', '', '後', 0, '営業日', 1, '基準日そのもの'],
    [80, '結果の整理・記録作成', '会議', '後', 1, '営業日', 1, ''],
    [90, '決裁の起案・システム入力', '結果の整理・記録作成', '後', 1, '営業日', 1, '起案日'],
    [100, '決裁完了（見込）', '決裁の起案・システム入力', '後', 3, '営業日', 2, '起案からN営業日。実績に合わせて調整する'],
    [110, '通知書の印刷', '決裁完了（見込）', '後', 1, '営業日', 1, ''],
    [120, '封入封緘・点検', '通知書の印刷', '後', 1, '営業日', 1, '二人体制で突合'],
    [130, '通知書の発送', '封入封緘・点検', '後', 1, '営業日', 1, '到達日を意識して逆算する'],
    [140, '台帳更新・報告用データ反映', '通知書の発送', '後', 2, '営業日', 2, '']
  ];
  appendRows_(SHEET.TEMPLATE, monthly.map(function (r) {
    return {
      '業務ID': 'W1', '工程No': r[0], '工程名': r[1], '基準': r[2], '方向': r[3], '日数': r[4],
      '単位': r[5], '休日補正': '前営業日', '担当': '', 'リマインド営業日前': r[6], '備考': r[7]
    };
  }));

  var annual = [
    [10, '案内の印刷・封入準備', '', '前', 20, '営業日', 5, ''],
    [20, '案内の一斉発送', '', '前', 10, '営業日', 3, '受付開始前に届くように'],
    [30, '受付開始', '', '後', 0, '営業日', 1, '基準日そのもの'],
    [40, '受付期間 中間点検（未提出者の把握）', '受付開始', '後', 30, '暦日', 3, ''],
    [50, '受付締切', '受付開始', '後', 60, '暦日', 5, '休日なら翌営業日に補正'],
    [60, '未提出者への勧奨通知', '受付締切', '後', 5, '営業日', 3, ''],
    [70, '審査・決裁（一斉分）完了', '受付締切', '後', 30, '営業日', 5, ''],
    [80, '新しい通知書の封入封緘', '審査・決裁（一斉分）完了', '後', 3, '営業日', 2, ''],
    [90, '新しい通知書の一斉発送', '新しい通知書の封入封緘', '後', 2, '営業日', 2, '有効期間の開始前に到達させる']
  ];
  appendRows_(SHEET.TEMPLATE, annual.map(function (r) {
    return {
      '業務ID': 'W2', '工程No': r[0], '工程名': r[1], '基準': r[2], '方向': r[3], '日数': r[4],
      '単位': r[5], '休日補正': '翌営業日', '担当': '', 'リマインド営業日前': r[6], '備考': r[7]
    };
  }));

  var monthly2 = [
    [10, '受付分の締切', '', '前', 15, '営業日', 3, ''],
    [20, '添付書類の確認・会議資料作成', '', '前', 8, '営業日', 3, ''],
    [30, '会議', '', '後', 0, '営業日', 1, ''],
    [40, '決裁起案', '会議', '後', 2, '営業日', 1, ''],
    [50, '通知書の発送', '決裁起案', '後', 5, '営業日', 2, '']
  ];
  appendRows_(SHEET.TEMPLATE, monthly2.map(function (r) {
    return {
      '業務ID': 'W3', '工程No': r[0], '工程名': r[1], '基準': r[2], '方向': r[3], '日数': r[4],
      '単位': r[5], '休日補正': '前営業日', '担当': '', 'リマインド営業日前': r[6], '備考': r[7]
    };
  }));
}
